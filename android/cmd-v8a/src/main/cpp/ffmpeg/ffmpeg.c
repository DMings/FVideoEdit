...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost, int unqueue) {
    ...
    cur_time = (float) (pkt->pts * av_q2d(ost->st->time_base));
    if (cur_time > progress_time) {
        progress_time = cur_time;
//        LOGI("progress_time: %f", progress_time);
    }
    if (debug_ts) {
